...
describe("Registry Program - GridTokenXClient", () => {
  ...
  describe("User Registration", () => {
    ...
    it("should validate user exists", async () => {
      const isValid = await client.isValidUser(
        client.getWalletAddress().toBase58()
      );
    ...
    it("should get all users", async () => {
      ...
      const ourUser = users.find(
        (u) => u.authority === client.getWalletAddress().toBase58()
      );
  ...
  describe("Meter Registration", () => {
    ...
    it("should register a solar meter", async () => {
      ...
      if (meterAccount) {
        ...
        expect(meterAccount.owner).to.equal(
          client.getWalletAddress().toBase58()
        );
  ...
  describe("User Status Management", () => {
    ...
    before(() => {
      userAddress = client.getWalletAddress().toBase58();
    });
