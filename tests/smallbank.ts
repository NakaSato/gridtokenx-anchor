...
describe("Smallbank Benchmark", () => {
    ...
    const [customerPda] = PublicKey.findProgramAddressSync([Buffer.from("sb_customer"), customerId.toArrayLike(Buffer, "le", 8)], program.programId);
    const [savingsPda] = PublicKey.findProgramAddressSync([Buffer.from("sb_savings"), customerId.toArrayLike(Buffer, "le", 8)], program.programId);
    const [checkingPda] = PublicKey.findProgramAddressSync([Buffer.from("sb_checking"), customerId.toArrayLike(Buffer, "le", 8)], program.programId);

    const [customer2Pda] = PublicKey.findProgramAddressSync([Buffer.from("sb_customer"), customerId2.toArrayLike(Buffer, "le", 8)], program.programId);
    const [checking2Pda] = PublicKey.findProgramAddressSync([Buffer.from("sb_checking"), customerId2.toArrayLike(Buffer, "le", 8)], program.programId);
    const [savings2Pda] = PublicKey.findProgramAddressSync([Buffer.from("sb_savings"), customerId2.toArrayLike(Buffer, "le", 8)], program.programId);
    ...
    it("Creates a Smallbank account", async () => {
        await program.methods.smallbankCreateAccount(
            customerId,
            "Alice",
            new BN(1000),
            new BN(500)
    ...
    it("Performs TransactSavings", async () => {
        await program.methods.smallbankTransactSavings(new BN(500)).accounts({
            savings: savingsPda,
            authority: authority.publicKey
        }).rpc();
    ...
    it("Performs DepositChecking", async () => {
        await program.methods.smallbankDepositChecking(new BN(200)).accounts({
            checking: checkingPda,
            authority: authority.publicKey
        }).rpc();
    ...
    it("Performs SendPayment", async () => {
        await program.methods.smallbankSendPayment(new BN(300)).accounts({
    ...
    it("Performs WriteCheck", async () => {
        await program.methods.smallbankWriteCheck(new BN(100)).accounts({
            checking: checkingPda,
            authority: authority.publicKey
        }).rpc();
    ...
    it("Performs Amalgamate", async () => {
        await program.methods.smallbankAmalgamate().accounts({
            savings: savingsPda,
            checking: checkingPda,
            authority: authority.publicKey
        }).rpc();
