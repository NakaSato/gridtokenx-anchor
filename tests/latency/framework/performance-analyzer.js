...
export class PerformanceAnalyzer {
     ...
     */
    calculateMetrics(operation, measurements, duration, errors = []) {
     ...
     */
    calculateThroughput(operations, duration) {
        return {
            operations,
            duration,
     ...
     */
    calculateErrorStats(errors, totalOperations) {
     ...
     */
    detectOutliers(latencies, mean, stdDev) {
        const threshold = this.config.outlierThreshold * stdDev;
        return latencies.filter(latency => Math.abs(latency - mean) > threshold);
    }
    /**
     ...
     */
    analyzeTrends(measurements, windowSize = 100) {
     ...
     */
    detectRegression(currentMetrics, baselineOperation) {
    ...
    generateReport(measurements) {
        ...
        const summary = {
            ...
            standardDeviation: Math.sqrt(latencies.reduce((sum, l) => sum + Math.pow(l - (latencies.reduce((s, li) => s + li, 0) / latencies.length), 2), 0) / latencies.length),
     ...
     */
    groupMeasurementsByProgram(measurements) {
     ...
     */
    generateRecommendations(summary, trends, regressions) {
