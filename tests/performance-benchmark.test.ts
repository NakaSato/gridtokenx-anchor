...
describe("Performance Benchmark - Transaction Latency & Throughput", () => {
  ...
  function printReport(testName: string, metrics: PerformanceMetrics) {
    ...
    console.log(`  Throughput:              ${metrics.throughput.toFixed(2)} tx/sec`);
    console.log("-".repeat(70));
  ...
  describe("Sequential Transaction Loop", () => {
    it("should measure latency for 50 sequential token transfers", async function(this: Mocha.Context) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        try {
          await client.transferTokens(recipient.publicKey.toString(), transferAmount);
    ...
    it("should measure latency for 30 sequential order placements", async function(this: Mocha.Context) {
      this.timeout(300000); // 5 minutes
    ...
    it("should measure latency for 20 sequential REC validator additions", async function(this: Mocha.Context) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        try {
          ...
          await client.addRecValidator(validator.publicKey.toString(), authorityName);
          const txEnd = Date.now();
          const latency = txEnd - txStart;
          latencies.push(latency);
    ...
    it("should measure throughput for 100 token transfers in batches of 10", async function(this: Mocha.Context) {
      ...
      for (let batch = 0; batch < totalIterations / batchSize; batch++) {
        ...
        for (let i = 0; i < batchSize; i++) {
          const txStart = Date.now();
          const promise = client.transferTokens(recipient.publicKey.toString(), transferAmount)
            .then(() => {
            ...
            .catch((error: any) => {
              failed++;
              console.log(`  ✗ Batch ${batch} tx ${i} failed: ${error.message}`);
            });
        ...
        console.log(`  ✓ Completed batch ${batch + 1}/${totalIterations / batchSize} (${latencies.length} successful)`);
      }
    ...
    it("should measure throughput for 60 order placements in batches of 5", async function(this: Mocha.Context) {
      ...
      for (let batch = 0; batch < totalIterations / batchSize; batch++) {
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          const promise = client.placeOrder(side, energyAmount, price)
            .then(() => {
              const latency = Date.now() - txStart;
              latencies.push(latency);
            })
            .catch((error: any) => {
              failed++;
              console.log(`  ✗ Batch ${batch} order ${i} failed: ${error.message}`);
            });
  ...
  describe("Sustained Load Test", () => {
    it("should handle sustained load of 200 mixed transactions", async function(this: Mocha.Context) {
      ...
      console.log(`  ✓ Throughput: ${metrics.throughput.toFixed(2)} tx/sec ${metrics.throughput > 0.5 ? '(PASS)' : '(FAIL)'}`);
      console.log(`  ✓ Avg Latency: ${metrics.avgLatency.toFixed(2)} ms ${metrics.avgLatency < 15000 ? '(PASS)' : '(FAIL)'}`);
      console.log(`  ✓ P95 Latency: ${metrics.p95Latency.toFixed(2)} ms ${metrics.p95Latency < 20000 ? '(PASS)' : '(FAIL)'}`);
      console.log(`  ✓ Success Rate: ${((metrics.successfulTransactions / metrics.totalTransactions) * 100).toFixed(2)}% ${metrics.successfulTransactions / metrics.totalTransactions > 0.7 ? '(PASS)' : '(FAIL)'}\n`);
    });
  });
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        try {
          ...
          if ((i + 1) % 50 === 0) {
            ...
            console.log(`  ✓ ${i + 1}/${iterations} | Throughput: ${currentMetrics.throughput.toFixed(2)} tx/sec | Avg Latency: ${currentMetrics.avgLatency.toFixed(2)} ms`);
          }
        } catch (error: any) {
          failed++;
          if (failed % 10 === 0) {
  ...
  describe("Latency Distribution Analysis", () => {
    it("should analyze latency distribution across 100 transfers", async function(this: Mocha.Context) {
      ...
      console.log(`  0-1s:     ${buckets['0-1s'].toString().padStart(4)} (${((buckets['0-1s'] / latencies.length) * 100).toFixed(1)}%)`);
      console.log(`  1-2s:     ${buckets['1-2s'].toString().padStart(4)} (${((buckets['1-2s'] / latencies.length) * 100).toFixed(1)}%)`);
      console.log(`  2-5s:     ${buckets['2-5s'].toString().padStart(4)} (${((buckets['2-5s'] / latencies.length) * 100).toFixed(1)}%)`);
      console.log(`  5-10s:    ${buckets['5-10s'].toString().padStart(4)} (${((buckets['5-10s'] / latencies.length) * 100).toFixed(1)}%)`);
      console.log(`  10s+:     ${buckets['10s+'].toString().padStart(4)} (${((buckets['10s+'] / latencies.length) * 100).toFixed(1)}%)\n`);
