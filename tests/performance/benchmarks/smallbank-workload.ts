 ...
 * 
 * Simplified banking benchmark for baseline comparison.
 * Used alongside TPC-C to establish consensus stress testing baseline.
 * 
 * Transactions:
 * - TransactSavings (15%) - Update savings balance
 * - DepositChecking (15%) - Deposit to checking
 * - SendPayment (25%)     - Transfer between accounts
 * - WriteCheck (25%)      - Debit checking account
 * - Amalgamate (10%)      - Combine accounts
 * - Balance (10%)         - Check balance (read-only)
 * 
 * Reference: Blockbench framework specification
 */
...
export interface SmallbankConfig {
    ...
    hotspotPercentage: number;  // % of accounts receiving most traffic
    hotspotFraction: number;    // Fraction of traffic to hotspots
}
...
export class SmallbankWorkload {
    ...
    constructor(config: Partial<SmallbankConfig> = {}) {
        this.config = {
            accountCount: config.accountCount || 100000,
    ...
    /**
     * Generate account ID with hotspot distribution
     */
    private generateAccountId(): number {
        ...
        if (Math.random() < this.config.hotspotFraction) {
            // Access hotspot accounts
            return Math.floor(Math.random() * hotspotAccounts) + 1;
        } else {
            // Access cold accounts
            return Math.floor(Math.random() * (this.config.accountCount - hotspotAccounts)) + hotspotAccounts + 1;
        }
    }

    /**
     * Select transaction type based on Smallbank mix
     */
    ...
    /**
     * Generate a Smallbank transaction
     */
    generateTransaction(): SmallbankTransaction {
        ...
        switch (type) {
            ...
            case 'BALANCE':
                // Read-only, no additional params
                break;
        }
    ...
    async executeTransaction(tx: SmallbankTransaction): Promise<SmallbankResult> {
        ...
        switch (tx.type) {
            case 'SEND_PAYMENT':
            case 'AMALGAMATE':
                baseLatency = 3 + Math.random() * 5; // Multi-key operations
                conflict = Math.random() < 0.015;
                break;
            case 'BALANCE':
                baseLatency = 1 + Math.random() * 2; // Read-only
                break;
            default:
                baseLatency = 2 + Math.random() * 3; // Single-key writes
        ...
        if (conflict) {
            success = Math.random() > 0.2; // 80% retry success
        }

        const result: SmallbankResult = {
    ...
    async run(options: {
        ...
        console.log('\n╔════════════════════════════════════════════════════════════╗');
        console.log('║     Smallbank Benchmark for GridTokenX                     ║');
        console.log('║     Baseline Consensus Stress Test                         ║');
        console.log('╚════════════════════════════════════════════════════════════╝\n');

        console.log(`Configuration:`);
        console.log(`  Accounts: ${this.config.accountCount}`);
        console.log(`  Hotspot: ${this.config.hotspotPercentage}% accounts, ${this.config.hotspotFraction * 100}% traffic`);
...
const isMainModule = import.meta.url === `file://${process.argv[1]}` ||
    process.argv[1]?.endsWith('smallbank-workload.ts');

if (isMainModule) {
    const workload = new SmallbankWorkload({
        accountCount: 10000,
