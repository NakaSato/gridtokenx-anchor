  ...
  describe("500+ Trades/Minute Simulation", () => {
    it("Should handle 500 trades per minute sustained for 5 minutes", async () => {
      ...
      try {
        ...
        // Assertions
        expect(results.metrics.successfulTransactions, `Should achieve at least 95% success rate. Got: ${results.metrics.successfulTransactions}`).to.be.at.least(targetTradesPerMinute * testDurationMinutes * 0.95);

        expect(results.metrics.throughput, `Should maintain at least 90% of target TPS. Got: ${results.metrics.throughput.toFixed(2)} TPS`).to.be.at.least(targetTradesPerMinute * 0.9);

        expect(results.metrics.averageLatency, `Average latency should be under 1s. Got: ${results.metrics.averageLatency.toFixed(2)}ms`).to.be.below(1000);
  ...
  describe("10,000+ Active Orders in Order Book", () => {
    it("Should handle 10,000 concurrent active orders", async () => {
      ...
      try {
        ...
        await framework.saveResults(sessionId, finalResults);
    ...
    it("Should maintain performance under sustained load (1+ hour)", async () => {
      ...
      try {
        ...
        const firstHalfLatency = results.transactions
          ...
          .reduce((sum: number, t: any) => sum + t.latency, 0) /
