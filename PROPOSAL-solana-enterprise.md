...
| **Token-2022 program** | Confidential balances, transfer hooks, permanent delegate (PYUSD case) | Compliance-grade stablecoin settlement (THBC peg) — extension headroom for confidential/compliance transfers | `treasury/` THBC runs on Token-2022 (`scripts/init-treasury.ts:65`) via generic `token_interface`; **no extensions wired yet** (`programs/treasury/src/lib.rs:897-906`) |
...
- **Custom token standard** — THBC is already minted on the **Token-2022 program** (`scripts/init-treasury.ts:65` passes `TOKEN_2022_PROGRAM_ID`; treasury code is program-agnostic via `anchor_spl::token_interface`, `programs/treasury/src/lib.rs:4-6`). The mint is currently a **base Token-2022 mint with no extensions enabled** (`programs/treasury/src/lib.rs:897-906` — `mint::decimals=6, mint::authority=treasury`, nothing else). Being on the 2022 program gives the *headroom* to adopt the compliance extensions regulators expect, each of which is a **future, un-wired** task:
  ...
  - *Permanent delegate* → freeze/seize authority for regulatory enforcement on the THBC stablecoin. **Not yet wired.**
...
- **PayPal PYUSD → Token-2022 stablecoin.** PYUSD adopted confidential balances + transfer hooks + permanent delegate. GridTokenX's **THBC** (THB-pegged, treasury-PDA mint authority) is on the **same program** (Token-2022) and follows the same playbook for a baht-denominated energy-settlement stablecoin, with reserve-attestation peg invariants already enforced in `treasury/`. **Gap vs PYUSD:** THBC enables *none* of those three extensions yet — adopting them is scoped as future work in §6.
...
2. **Token-2022 extension adoption** *(new task — scope, then implement through normal treasury design review)*
   - *Verified starting state:* THBC is a base Token-2022 mint with **no extensions enabled** (`programs/treasury/src/lib.rs:897-906`); treasury is program-agnostic via `anchor_spl::token_interface` (`lib.rs:4-6`); mint created under `TOKEN_2022_PROGRAM_ID` (`scripts/init-treasury.ts:65`).
   - *2a. Init-path rework.* Mint is currently created by Anchor's `mint::` constraint, which produces a **base mint with no extension space**. Extensions must be initialized *before* `InitializeMint` on a pre-sized account → likely move THBC creation off the `mint::` macro to an explicit CPI init (`ExtensionType` space calc + `initialize_*` instruction), or a TS-side pre-init in `init-treasury.ts`. Decide which.
   ...
   - *2c. Transfer hooks* — on-chain compliance automation at transfer; requires a companion hook program + extra-account-meta list. Highest blast radius (every THBC transfer path, incl. swap/redeem/settlement, must pass hook accounts).
