...
Running the suite requires the program's compiled `tpc_benchmark` BPF object. Because each `programs/*` directory is its own crate rather than a member of a root workspace, Anchor 1.0 emits the object under the sub-workspace's own `target/deploy/`; it must be copied into the root `target/deploy/` before deploying, which is the path required for the test validator to load and deploy the program. Run the suite with `anchor test` (or `npm run test:tpc-stress`). The benchmark caveat applies: per `BENCHMARKS.md:45-48`, end-to-end latency reflects block time and client submission overhead, so the load-independent compute-unit figures are the appropriate measure of on-chain efficiency.
