...
2. **Periodic uniform-price batch auction.** `clear_auction` (`lib.rs:893`) builds aggregate supply and demand curves, locates the clearing price via `find_clearing_point` (`lib.rs:1841`), and matches all eligible orders at a single uniform clearing price (`lib.rs:893-1038`). Settlement of the resulting matches is separated into `execute_auction_matches` (`lib.rs:1048`).
...
`ZoneCapacity` (`zone_market.rs:47-54`). PDA seed: `[b"zone_capacity", zone_market.key()]` (`lib.rs:1568`), created by `init_zone_capacity` (`lib.rs:199-207`). Holds the cross-zone `committed_flow` counter split OUT of `ZoneMarket` so the settle hot path keeps `ZoneMarket` read-only: only cross-zone (wheeling) settlements pass this account `mut` and bump the counter; intra-zone settles omit it and do not serialize on the zone book (`zone_market.rs:41-46`). Fields: `zone_market` (parent binding), `committed_flow`, `bump`, `_padding[7]`.
...
**`init_zone_capacity`** (`lib.rs:199-207`). Signer: `payer`. One-time creation of the per-zone `ZoneCapacity` PDA (§3.4) that carries the cross-zone `committed_flow` counter.
