...
> **⚠️ STATUS: PROPOSED — NOT YET IMPLEMENTED.** This document specifies a *revised* collateral model that is not present in the current on-chain code. The deployed system uses a **GRX** validator bond (`registry::stake_grx`, gated by `MIN_VALIDATOR_STAKE`) plus a separate **GRX yield-staking** product in the treasury (`acc_reward_per_share`/`reward_pool`). Slashing today sends the GRX bond to a single configured `slash_destination` (`registry::slash_validator` → typically the treasury `reward_vault`, redistributed to stakers via `fund_rewards`). The THBG-denominated bond, the no-reward design, capped victim compensation, partial slashing with top-up, and the Merkle/challenge-response adjudication described below **do not exist in code yet** — they are a forward design. Treat all present-tense statements as the target design, not current behavior.
