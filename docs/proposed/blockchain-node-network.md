...
> **⚠️ STATUS: MIXED — network/account layout implemented; Tier-2 settlement validity PROPOSED.** The permissioned-Solana model, the treasury program, the PDA/vault layout (§5), and the non-custodial settlement call are real. The **Tier-2 settlement-validity layer** (§3.2, §6, §7) — Merkle commitment of matches, challenge-response, fraud proofs, and a **per-zone `settlement_record` PDA holding root/VAT/zone-id** — is **not implemented**: `treasury::record_settlement(ctx, value: u64)` records a settlement *value* only and bumps a single global `total_settled_thbg` counter. Aggregator collateral is currently a **GRX** bond, not THBG (see the collateral-and-slashing doc, also PROPOSED). §2's "GRX collateral" conflicts with the slashing doc's THBG bond — these need reconciling. Treat Tier-2 / Merkle / challenge-window statements as target design.
