...
GridTokenX has been rigorously tested using industry-standard TPC benchmarks adapted for blockchain platforms.

\subsection{Benchmark Results}

\begin{table}[H]
\centering
\caption{TPC-C Performance Results (Local Validator)}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
...
\begin{equation}
\text{Trust Premium} = \frac{\text{Blockchain Latency}}{\text{Centralized Baseline}} = \frac{11.30\text{ms}}{2\text{ms}} = 5.65\times
\end{equation}

This 5.65x overhead is negligible for energy trading applications where settlement traditionally takes days or weeks.

\subsection{Transaction Mix}

\begin{table}[H]
\centering
\caption{Benchmark Transaction Distribution}
\begin{tabular}{lcc}
\toprule
\textbf{Operation} & \textbf{Count} & \textbf{Percentage} \\
\midrule
Create Order & 10,558 & 44.3\% \\
Token Transfer & 10,353 & 43.5\% \\
Execute Trade & 973 & 4.1\% \\
Balance Check & 976 & 4.1\% \\
Order Status & 957 & 4.0\% \\
\bottomrule
\end{tabular}
\end{table}

\begin{center}
\fbox{\parbox{0.9\textwidth}{\textbf{Key Finding:} With 352 TPS capacity and 11ms latency, GridTokenX can support 10,000+ prosumer households with a 32x safety margin.}}
\end{center}
