...
sequenceDiagram
    ...
    SM->>SM: Generate Reading (kWh)
    SM->>SM: Sign with Ed25519
    SM->>API: Submit Signed Reading
    API->>DB: Store Reading
    API->>Oracle: Validate Reading
    Oracle-->>API: Validation Proof
    API->>Token: Mint GRID Tokens
    Token-->>API: Tokens Minted
    
    Note over API,Token: Prosumer now has GRID tokens
    
    API->>Trading: Create Sell Order
    Trading->>Trading: Lock Tokens in Escrow
    
    Buyer->>Trading: Match Order
    Trading->>Trading: Atomic Settlement
    Trading-->>Buyer: Transfer GRID Tokens
    Trading-->>API: Transfer GRX Payment
    
    Note over Trading,Buyer: Trade Complete ✅
```

```mermaid
flowchart TD
    A[Smart Meter Reading] --> B[Ed25519 Signing]
    B --> C[API Gateway]
    C --> D{Validation}
    D -->|Valid| E[Store in DB]
    D -->|Invalid| F[Reject]
    E --> G[Oracle Validation]
    G --> H[Mint GRID Tokens]
    H --> I[Create Sell Order]
    I --> J[Lock in Escrow]
    J --> K{Order Matched?}
    K -->|Yes| L[Atomic Settlement]
    K -->|No| M[Wait in Order Book]
    L --> N[Transfer Tokens]
    L --> O[Transfer Payment]
    N --> P[Trade Complete]
    O --> P
```
