...
// [b"governance_config"] PDA. `issue_erc` mints REC to producers; `retire_rec` burns them.
//
// Run AFTER init-governance.ts (the governance_config PDA must already exist):
...
async function main() {
  ...
  try {
    const sig = await governance.methods
      ...
      .rpc();
    console.log('  ✅ REC mint initialized (6 decimals, authority = governance_config PDA):', sig);
  } catch (e: any) {
