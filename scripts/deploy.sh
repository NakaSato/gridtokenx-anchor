...
if [ "$WALLET_BALANCE" -lt "$MIN_BALANCE" ]; then
...
for program in $PROGRAMS; do
    ...
    if anchor deploy --program-path "$PROGRAM_PATH" --program-name "$program"; then
...
cat > "target/reports/deployment-${ENVIRONMENT}-$(date +%Y%m%d%H%M%S).json" << EOF
{
...
for program in "${DEPLOYED_PROGRAMS[@]}"; do
    PROGRAM_ID=$(jq -r ".programs[] | select(.name==\"$program\") | .program_id" "$CONFIG_FILE")
    echo "$comma    {\"name\": \"$program\", \"programId\": \"$PROGRAM_ID\"}" >> "target/reports/deployment-${ENVIRONMENT}-$(date +%Y%m%d%H%M%S).json"
    comma=","
done

# Add failed programs to report
if [ $FAILED_COUNT -gt 0 ]; then
    echo "," >> "target/reports/deployment-${ENVIRONMENT}-$(date +%Y%m%d%H%M%S).json"
    comma=""
    for program in "${FAILED_PROGRAMS[@]}"; do
        echo "$comma    {\"name\": \"$program\", \"status\": \"failed\"}" >> "target/reports/deployment-${ENVIRONMENT}-$(date +%Y%m%d%H%M%S).json"
        comma=","
    done
fi
...
echo "Success Rate: $(( SUCCESS_COUNT * 100 / TOTAL_PROGRAMS ))%"
echo ""
...
if [ $FAILED_COUNT -eq 0 ]; then
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Run ./scripts/verify-deployment.sh $ENVIRONMENT to verify deployment"
